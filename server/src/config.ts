 ...
 * 选择要加载的 env 文件。
 *
 * 默认读 `.env`；设了 `DAS_ENV=<name>` 就改读 `.env.<name>`（例如 `DAS_ENV=pre`
 * 读 `.env.pre`），用来在多个上游环境（生产 / 预发 / 日常）之间切换而不动代码、
 ...
 * （`MOCK=1 DAS_ENV=pre npm run dev`）仍然优先于文件，单条命令临时改一个值照样生效。
 *
...
function loadEnvFile(): void {
  ...
  // 两个位置都找：仓库根（正常用法）与 cwd（有人单独在 server/ 目录里跑）。
...
function printMissingCredentials(): void {
  const lines = [
    ...
    '     MOCK 模式下完全不调真实接口，回放本仓库自带的合成样例帧流，无需任何凭证。',
    '',
    '  B. 要调真实接口',
...
export function describeConfig(cfg: AppConfig): string {
  const lines = [
    `mode          : ${cfg.mock ? 'MOCK（回放合成样例帧流，不调真实接口）' : 'LIVE（调用真实 OpenAPI）'}`,
  ];
  if (cfg.mock) {
    lines.push(
