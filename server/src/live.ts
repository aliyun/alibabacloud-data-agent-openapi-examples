 ...
 * （server/test/live-create.test.ts）。
 *
...
function toSessionSummary(row: unknown): SessionSummary | undefined {
  ...
  return {
    sessionId,
    /**
     * SessionTitle = 首条 prompt 原文，里面可能带着注入的校验码说明；
     * 剥离由前端用 `stripMarkerInstruction` 做（注入与剥离必须同源，都在 shared）。
 ...
 *
 * 注意 `outboundText` 是**已经注入过 marker 的完整文本**。注入在服务端做，
 * 这样"注入的内容"与"真正发出去的东西"之间不隔一层网络。
 */
