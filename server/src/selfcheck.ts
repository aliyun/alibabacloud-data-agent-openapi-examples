  ...
  if (cfg.mock) {
    ...
    return {
      step: done('list-agents', api, started, true, env.requestId, undefined, mockNote(
        `回放合成样例：返回 ${names.length} 个 agent（${names.join(', ')}）。`,
        api,
      )),
    };
  }
...
async function stepTokenUsage(
  ...
  if (cfg.mock) {
    ...
    return done('token-usage', api, started, env.result !== undefined, env.requestId, undefined, mockNote(
      `回放合成样例：TotalTokens=${env.result?.TotalTokens ?? '?'}，PromptTokens=${env.result?.PromptTokens ?? '?'}。` +
        '样例里 PromptTokens 远大于用户发的那一句话——大头是 agent 的 system prompt 与技能上下文，不是你的输入。' +
        '别把任何一次观测当规律：不同账号/会话差异很大，新建空会话实测也可能是 0。',
      api,
    ));
  }
  ...
  /**
   * 这里刻意不写死"空会话就有五万级 PromptTokens"：不同账号/会话的实测差异很大，
   * 既有新建空会话就五万级的（system prompt 与技能上下文），也有空会话为 0 的。
  ...
  const tokenNote =
    promptTokens >= 10_000
      ...
      : `这次 PromptTokens=${promptTokens}。别把它当常量：实测中既有新建空会话就五万级的` +
        '（agent 的 system prompt 与技能上下文），也有空会话是 0 的。两种都不是你的输入。';
