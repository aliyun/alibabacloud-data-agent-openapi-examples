...
describe('buildCreateSessionRequest — wire 序列化（防 Params:{} 回归）', () => {
  ...
  it('sessionSource 与 SessionTags 落在 Meta.Config 下，且 SessionTags 是对象数组而不是字符串数组', () => {
    const map = buildCreateSessionRequest(cfg({ sessionSource: 'data-agent-openapi-demo' })).toMap() as WireMap;
    expect(map.Params?.Meta?.Config?.SessionSource).toBe('data-agent-openapi-demo');
    expect(map.Params?.Meta?.Config?.SessionTags).toEqual([
      { SessionTagCode: 'data-agent-openapi-demo' },
    ]);
  });
